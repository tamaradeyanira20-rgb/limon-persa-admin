...
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .badge-active{background:rgba(190,242,100,.15);color:var(--lime)}
    .badge-inactive{background:rgba(100,100,100,.15);color:var(--muted)}
    .tab-btn{padding:10px 14px;border:none;font-size:12px;font-weight:700;cursor:pointer;transition:all .2s;font-family:'Syne',sans-serif;white-space:nowrap;border-radius:0;border-bottom:2px solid transparent;background:transparent}
    ...
    .modal{background:var(--card2);border:1px solid var(--border);border-radius:16px;padding:24px;width:100%;max-width:500px;max-height:90vh;overflow-y:auto}
...
const Modal = ({ title, onClose, children, wide }) => (
  <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
    <div className="modal fade-up" style={{ maxWidth: wide ? 700 : 500 }}>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 360 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 13 }}>Limón Persa</p>
        </div>
        <div className="card">
          <label className="label">Contraseña</label>
          <input className="input-field" type="password" placeholder="Contraseña de admin" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && (pw === ADMIN_PASSWORD ? onLogin() : setErr("Incorrecta"))} style={{ marginBottom: 12 }} />
          {err && <p style={{ color: "var(--danger)", fontSize: 13, marginBottom: 12 }}>{err}</p>}
          <button className="btn btn-lime" onClick={() => pw === ADMIN_PASSWORD ? onLogin() : setErr("Contraseña incorrecta")} style={{ width: "100%", padding: 12 }}>Entrar</button>
        </div>
      </div>
    </div>
  );
...
// ─── USUARIOS (con productos y referidos) ─────────────────────
const Users = () => {
  const [users, setUsers] = useState([]); const [loading, setLoading] = useState(true); const [search, setSearch] = useState("");
  const [modal, setModal] = useState(null);
  const [inputVal, setInputVal] = useState(""); const [saving, setSaving] = useState(false); const [msg, setMsg] = useState("");
  const [detailUser, setDetailUser] = useState(null);
  const [userProducts, setUserProducts] = useState([]); const [userRefs, setUserRefs] = useState([]);
  ...
  const openModal = (type, user) => { setModal({ type, user }); setInputVal(""); setMsg(""); };

  const openDetail = async (user) => {
    setDetailUser(user); setLoadingDetail(true); setUserProducts([]); setUserRefs([]);
    ...
    // Para cada referido, buscar si tiene compras activas
  ...
  return (
    <div>
                ...
                <tr key={u.id}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      ...
                      <button className="btn btn-lime" style={{ fontSize: 11, padding: "5px 8px" }} onClick={() => openModal("balance", u)}>💰</button>
                      <button className="btn btn-blue" style={{ fontSize: 11, padding: "5px 8px" }} onClick={() => openModal("spins", u)}>🎰</button>
                      <button className="btn btn-purple" style={{ fontSize: 11, padding: "5px 8px" }} onClick={() => openModal("password", u)}>🔑</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {detailUser && (
        <Modal title={`👁️ Detalle — ${detailUser.phone}`} onClose={() => setDetailUser(null)} wide>
          {loadingDetail ? <div className="spinner" style={{ margin: "20px auto" }} /> : (
            <div className="gap">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[["Saldo", fmt(detailUser.balance)], ["Giros", detailUser.spins || 0], ["Código", detailUser.referral_code], ["Registro", new Date(detailUser.created_at).toLocaleDateString("es-MX")]].map(([k, v]) => (
                  <div key={k} className="card" style={{ padding: "10px 14px" }}>
                    <p style={{ color: "var(--muted)", fontSize: 11 }}>{k}</p>
                    <p style={{ fontWeight: 700, color: "var(--lime)", fontSize: 15 }}>{v}</p>
                  </div>
                ))}
              </div>
              ...
              {/* Productos comprados */}
              <div>
                <p style={{ fontWeight: 700, marginBottom: 10, fontSize: 14 }}>📦 Productos comprados ({userProducts.length})</p>
              ...
              {/* Árbol de referidos */}
              <div>
                <p style={{ fontWeight: 700, marginBottom: 10, fontSize: 14 }}>👥 Referidos de {detailUser.phone} ({userRefs.length})</p>
                {userRefs.length === 0 ? <p style={{ color: "var(--muted)", fontSize: 13 }}>No ha referido a nadie todavía</p> : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {userRefs.map(r => (
                      <div key={r.id} className="card" style={{ padding: "12px 14px", borderLeft: r.purchases.length > 0 ? "3px solid var(--lime)" : "3px solid var(--muted)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div>
                            <p style={{ fontWeight: 600 }}>{r.phone}</p>
                            <p style={{ color: "var(--muted)", fontSize: 11 }}>Se registró: {new Date(r.created_at).toLocaleDateString("es-MX")}</p>
                          </div>
                        ...
                        {r.purchases.length > 0 && (
                          <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {r.purchases.map(p => (
                              <span key={p.id} style={{ background: "rgba(190,242,100,.1)", color: "var(--lime)", padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>📦 {p.products?.name}</span>
                            ))}
                          </div>
                        )}
                      </div>
...
const Products = () => {
  ...
  const [modal, setModal] = useState(null);
  const [f, setF] = useState({ name: "", price: "", daily_return: "", description: "", image_url: "" });
  const [saving, setSaving] = useState(false); const [msg, setMsg] = useState("");

  const load = useCallback(async () => { const d = await sb("products?order=id").catch(() => []); setProducts(d); setLoading(false); }, []);
  useEffect(() => { load(); }, [load]);

  const openEdit = (p) => { setF({ name: p.name, price: p.price, daily_return: p.daily_return, description: p.description || "", image_url: p.image_url || "" }); setModal({ mode: "edit", product: p }); setMsg(""); };
  const openNew = () => { setF({ name: "", price: "", daily_return: "", description: "", image_url: "" }); setModal({ mode: "new" }); setMsg(""); };

  const save = async () => {
    setSaving(true); setMsg("");
    try {
      if (!f.name || !f.price || !f.daily_return) throw new Error("Completa nombre, precio y rendimiento");
      const body = { name: f.name, price: Number(f.price), daily_return: Number(f.daily_return), description: f.description, image_url: f.image_url };
      if (modal.mode === "edit") {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
          {products.map(p => (
            <div key={p.id} className="card">
              {p.image_url && <img src={p.image_url} alt={p.name} style={{ width: "100%", height: 120, objectFit: "cover", borderRadius: 8, marginBottom: 12 }} onError={e => e.target.style.display = "none"} />}
              {!p.image_url && <div style={{ width: "100%", height: 80, background: "var(--bg)", borderRadius: 8, marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--muted)", fontSize: 12 }}>Sin imagen</div>}
              <h4 style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>{p.name}</h4>
              <p style={{ color: "var(--lime)", fontWeight: 700 }}>{fmt(p.price)}</p>
              <p style={{ color: "var(--gold)", fontSize: 13 }}>+{fmt(p.daily_return)}/día</p>
              {p.description && <p style={{ color: "var(--muted)", fontSize: 12, marginTop: 4 }}>{p.description}</p>}
              <button className="btn btn-blue" style={{ marginTop: 12, width: "100%", fontSize: 12 }} onClick={() => openEdit(p)}>✏️ Editar</button>
            </div>
          ))}
        </div>
      )}
        ...
        <Modal title={modal.mode === "edit" ? `✏️ Editar — ${modal.product?.name}` : "➕ Nuevo producto"} onClose={() => setModal(null)}>
          <div className="gap">
            ...
            <div>
              ...
              <input className="input-field" placeholder="https://ejemplo.com/imagen.jpg" value={f.image_url} onChange={e => setF(p => ({ ...p, image_url: e.target.value }))} />
              <p style={{ color: "var(--muted)", fontSize: 11, marginTop: 6 }}>Sube tu imagen a <b>imgur.com</b> o <b>imgbb.com</b> y pega el link aquí</p>
            </div>
            {f.image_url && (
              <div>
                <label className="label">Vista previa</label>
                <img src={f.image_url} alt="preview" style={{ width: "100%", height: 140, objectFit: "cover", borderRadius: 8 }} onError={e => e.target.style.display = "none"} />
              </div>
            )}
            {msg && <p style={{ color: msg.startsWith("✅") ? "var(--lime)" : "var(--danger)", fontSize: 13 }}>{msg}</p>}
            <div style={{ display: "flex", gap: 8 }}>
...
// ─── CUENTA BANCARIA (guarda en Supabase) ─────────────────────
const BankSettings = () => {
  ...
  useEffect(() => {
    sb("settings?key=like.bank_%25&select=key,value").then(rows => {
      ...
      setBank({ banco: map.banco || "", titular: map.titular || "", clabe: map.clabe || "", cuenta: map.cuenta || "" });
      setLoading(false);
  ...
  const save = async () => {
    ...
    try {
      ...
      await Promise.all([
        sb("settings?key=eq.bank_banco",   { method: "PATCH", body: JSON.stringify({ value: bank.banco,   updated_at: new Date().toISOString() }), prefer: "return=minimal" }),
        sb("settings?key=eq.bank_titular", { method: "PATCH", body: JSON.stringify({ value: bank.titular, updated_at: new Date().toISOString() }), prefer: "return=minimal" }),
        sb("settings?key=eq.bank_clabe",   { method: "PATCH", body: JSON.stringify({ value: bank.clabe,   updated_at: new Date().toISOString() }), prefer: "return=minimal" }),
        sb("settings?key=eq.bank_cuenta",  { method: "PATCH", body: JSON.stringify({ value: bank.cuenta,  updated_at: new Date().toISOString() }), prefer: "return=minimal" }),
      ]);
      setMsg("✅ Cuenta actualizada. Los usuarios la verán al instante.");
    } catch (e) { setMsg("❌ " + e.message); }
    setSaving(false);
  };
  ...
  return (
    <div>
      <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>🏦 Cuenta bancaria de depósitos</h3>
        ...
        <div className="card">
          ...
          {bank.banco && (
            <div style={{ marginTop: 20, background: "var(--bg)", borderRadius: 10, padding: 16, border: "1px solid var(--border)" }}>
              <p style={{ color: "var(--lime)", fontSize: 11, fontWeight: 700, marginBottom: 12, textTransform: "uppercase", letterSpacing: 1 }}>Vista previa</p>
  ...
  return (
    <div>
      ...
      <div className="warn-box" style={{ marginTop: 16 }}>
        <p style={{ color: "var(--gold)", fontSize: 12 }}>⚠️ Las probabilidades deben sumar exactamente 100%. Para que el iPhone nunca salga pon 0%.</p>
      </div>
    </div>
  );
};
